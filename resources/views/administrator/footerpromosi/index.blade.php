...
<div class="d-none">
    <form id="form-destroy" action="{{ route('footerpromosi.store') }}" method="post">
        @csrf
        @method('DELETE')
    </form>
</div>
...
<script>
    $('table').DataTable({
        ...
        autoWidth: false,
        ajax: {
            url: baseUrl('/footerpromosi/fetch'),
            headers: {
        ...
        columns: [{
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    if (data) {
                        return `<img src="{{ asset('/upload/footerpromosi') }}/${data}" alt="${row.nama}" style="width:160px; height:auto;">`;
                    } else {
        ...
        createdRow: function(row, data) {
            $(".action-edit", row).click(function(e) {
                const url = baseUrl('/footerpromosi/' + data.id + '/edit');
