...
@section('content')
    <div class="breadcrumb justify-content-between">
        <div class="d-md-flex">
            ...
            <ul>
                ...
                <li>Transaksi Detail</li>
            </ul>
        </div>
        <div>
            <a href="{{ route('transaksi_detail.create') }}" class="btn btn-success">
    ...
    <div class="d-none">
        <form id="form-destroy" action="{{ route('transaksi_detail.store') }}" method="post">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
@section('js')
    <script>
        $('table').DataTable({
            ...
            autoWidth: false,
            ajax: {
                url: baseUrl('/transaksi_detail/fetch'),
                headers: {
            ...
            createdRow: function(row, data) {
                $(".action-edit", row).click(function(e) {
                    const url = baseUrl('/transaksi_detail/' + data.id + '/edit');
